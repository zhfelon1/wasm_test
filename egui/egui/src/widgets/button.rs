...
impl ButtonPng {
    pub fn new(
        texture_id: TextureId,
        size: Vec2,
        text: impl Into<WidgetText>) -> Self {
...
impl Widget for ButtonPng {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let wrap_width = ui.available_width() - min_size.x;
        ...
        if ui.is_rect_visible(rect) {
            ...
            let visuals = ui.style().interact(&response);
            let text_pos = pos2(
                rect.min.x + min_size.x / 2.0 - text.size().x / 2.0,
                rect.center().y - 0.5 * text.size().y,
            );
